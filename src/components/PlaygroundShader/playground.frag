...
vec3 cell90(vec2 cellUV, vec3 uTargetColor, vec3 uSecondColor){
    ...
    return uTargetColor * clamp(clamp(quad + quad2, 0., 1.0) - (rim + core), 0.0, 1.0) + uSecondColor * (rim + core);
}
...
vec3 cell100(vec2 cellUV, vec3 uTargetColor, vec3 uSecondColor){
  ...
  float quad = 1.0 - step(remap.x + 1.0, remap.y);
    ...
    return outerRim * mix(uSecondColor, uBackground, 0.5) + uSecondColor * clamp(quads + quad2 - circs + rim, 0., 1.0) + (clamp(quad - outerRim - rim, 0., 1.0) + circs) * uTargetColor;
}
...
void main() {
  ...
  vec2 cellCenter = (gridCoords + 0.5) * uGridSize / uResolution;
  cellCenter.y = 1.0 - cellCenter.y;
  vec3 texColor = texture2D(uTexture, cellCenter).rgb;

  // Convert to grayscale (luminance)
  float gray = dot(texColor, vec3(0.299, 0.587, 0.114));

  // Map to cell pattern based on brightness
  vec3 cell = cellVal(gray, gridUV, uTargetColor, uSecondColor);

  // Output: cell on background where cell has content
  float cellAlpha = step(0.01, length(cell));
  gl_FragColor = vec4(mix(uBackground, cell, cellAlpha), 1.0);
}
